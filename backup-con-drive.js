    ...
    output.on('close', async () => {
      ...
      if (UPLOAD_TO_DRIVE) {
        try {
          ...
          const driveModule = require('./backup-google-drive');
          await driveModule.uploadToDrive(backupFilePath);
          await driveModule.cleanOldDriveBackups(MAX_BACKUPS);
        } catch (error) {
          console.error('⚠ No se pudo subir a Google Drive:', error.message);
          console.log('  El respaldo local se guardó correctamente');
        }
      }
...
async function writeBackupLog(fileName, size) {
  try {
    ...
    const driveStatus = UPLOAD_TO_DRIVE ? ' [✓ Drive]' : '';
    const logEntry = `[${now}] ${fileName} - ${size} MB${driveStatus}\n`;
...
function listBackups() {
  console.log('\n═══════════════════════════════════════════════');
  console.log('  Respaldos disponibles (LOCAL)');
  console.log('═══════════════════════════════════════════════');
